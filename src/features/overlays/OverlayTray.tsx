...
export function OverlayTray({ selectedId, onAdd, onDelete, collapsed: initialCollapsed }: Props) {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 z-30 flex flex-col bg-background/90 backdrop-blur-md border-t border-border rounded-t-2xl transition-all">
      {/* Toggle bar */}
      <button
        ...
        <div className="px-3 pb-3 space-y-2 max-h-[45vh] overflow-y-auto">
          ...
          {tab === "stickers" && (
            <div className="space-y-2">
              {stickers.length > 0 && (
                <div>
                  <p className="text-[10px] font-medium text-muted-foreground mb-1">Pack</p>
                  <div className="flex flex-wrap gap-1.5">
                    {stickers.map((url, i) => (
                      <button
                        key={i}
                        onClick={() => handleAddSticker(`stickers/${i}`)}
                        className="flex h-10 w-10 items-center justify-center rounded-lg bg-secondary hover:bg-secondary/80 overflow-hidden transition-colors"
                      >
              ...
              <div>
                <p className="text-[10px] font-medium text-muted-foreground mb-1">Emojis</p>
                <div className="flex flex-wrap gap-1.5">
                  {DEFAULT_EMOJIS.map((emoji) => (
                    <button
                      key={emoji}
                      onClick={() => handleAddSticker(emoji)}
                      className="flex h-10 w-10 items-center justify-center rounded-lg bg-secondary text-lg hover:bg-secondary/80 transition-colors"
                    >
                      {emoji}
                    </button>
                  ))}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {frames.map((url, i) => (
                    <button
                      ...
                      className="flex h-14 w-14 items-center justify-center rounded-lg bg-secondary hover:bg-secondary/80 overflow-hidden transition-colors"
                    >
