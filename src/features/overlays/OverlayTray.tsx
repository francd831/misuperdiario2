...
import { Sticker, Frame, ImageIcon, Type, Trash2, X } from "lucide-react";
...
export function OverlayTray({ selectedId, onAdd, onDelete }: Props) {
  ...
  return (
    <div className="relative z-30 flex flex-col">
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      {stickers.map((url, i) => (
                        <button
                          ...
                          className="flex aspect-square items-center justify-center rounded-xl bg-secondary/60 hover:bg-secondary active:scale-90 overflow-hidden transition-all duration-150"
                        >
                  ...
                  <div className="grid grid-cols-6 gap-2">
                    {DEFAULT_EMOJIS.map((emoji) => (
                      <button
                        ...
                        className="flex aspect-square items-center justify-center rounded-xl bg-secondary/60 text-2xl hover:bg-secondary active:scale-90 transition-all duration-150"
                      >
                        {emoji}
                      </button>
                    ))}
      ...
      <div className="flex items-center justify-around bg-card/95 backdrop-blur-xl border-t border-border px-2 py-1.5">
        ...
        {/* Delete button - appears when an overlay is selected */}
        {selectedId && (
          <button
            onClick={onDelete}
