...
export default function AdminPage() {
  ...
  if (!unlocked) return <main className="adult-study" style={studyStyle}><section className="adult-study__board adult-study__board--locked"><form className="adult-study__access-form" onSubmit={handleUnlock}><div className="adult-study__access-main"><Link className="adult-study__access-back" to="/profiles" aria-label="Volver"><ArrowLeft size={21} /></Link><LockKeyhole size={40} aria-hidden="true" /><h2>Introduce tu PIN</h2><input aria-label="PIN de adulto" aria-live="polite" inputMode="none" maxLength={4} placeholder="••••" type="password" value={accessPin} readOnly />{accessError && <p className="form-error" role="alert">{accessError}</p>}<button className="adult-study__primary" disabled={accessPin.length !== 4} type="submit">Entrar</button></div><div className="adult-study__keypad" aria-label="Teclado numérico">{["1","2","3","4","5","6","7","8","9"].map((digit) => <button key={digit} type="button" onClick={() => addAccessDigit(digit)}>{digit}</button>)}<button type="button" aria-label="Borrar todo" onClick={() => setAccessPin("")}><X /></button><button type="button" onClick={() => addAccessDigit("0")}>0</button><button type="button" aria-label="Borrar último número" onClick={() => setAccessPin((value) => value.slice(0, -1))}><Delete /></button></div></form></section></main>;
